...
export function TestimonialsSection() {
  ...
  return <section id="depoimentos" className="bg-background py-0">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {testimonials.map((testimonial, index) => <Card key={index} className="bg-gradient-card border-border hover:scale-105 transition-transform duration-300">
              <CardContent className="p-6">
                {/* Stars */}
                <div className="flex mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => <Star key={i} className="h-5 w-5 text-primary fill-current" />)}
                </div>
